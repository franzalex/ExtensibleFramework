...
Public Class HomeActivity
    ...
    Private Sub HomeActivity_Start(sender As Object, e As Core.StartEventArgs) Handles Me.Start
        ' Initialize control by getting all activity launcers in plugins currently 
        ' loaded anddisplay them in the ListView
        ...
        For Each plugin In DirectCast(Me.Parent, ActivityHostControl).Plugins
            For Each launcher In plugin.ActivityLaunchers

                Dim imagekey = plugin.UniqueID + "@" + launcher.ActivityID
                Dim launcherInfo = New String() {launcher.ActivityID, launcher.InitializationCommand}

                iml32px.Images.Add(imagekey, launcher.Icon)

                lvwHomeItems.Items.Add(New ListViewItem() With {.Text = launcher.Caption,
                                                                .ToolTipText = launcher.Description,
                                                                .ImageKey = imagekey,
                                                                .Tag = launcherInfo})
            Next
        Next
    ...
    Private Sub HomeActivity_Stop(sender As Object, e As StopEventArgs) Handles Me.Stop
        ' release reseources by clearing all the loaded items
    ...
    Private Sub lvwHomeItems_DoubleClick(sender As Object, e As EventArgs) Handles lvwHomeItems.DoubleClick
        ' run the activity associated with the double clicked activity launcer item
